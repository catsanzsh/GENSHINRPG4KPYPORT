...
class Player(FirstPersonController):
    ...
    def attack(self):
        if self.cooldowns['attack'] > 0: return
    ...
    def elemental_skill(self):
        if self.cooldowns['skill'] > 0: return
        
        ui_manager.show_feedback(f"{self.active_char_name}'s Skill!", self.active_character.char_data['color'])
        skill_effect = Entity(
            model='sphere', color=self.active_character.char_data['color'], scale=0.1,
            position=self.position + self.forward * 3, collider='sphere'
        )
    ...
    def elemental_burst(self):
        if self.cooldowns['burst'] > 0: return

        ui_manager.show_feedback(f"{self.active_char_name}'s Burst!", self.active_character.char_data['color'], scale=2.5)
        burst_effect = Entity(
            model='sphere', color=self.active_character.char_data['color'],
            scale=0.1, position=self.position, alpha=0.5
        )
    ...
    def update(self):
        super().update()
        if self.y < -10: self.position = (0, 20, 0)
...
class Enemy(Entity):
    def __init__(self, position, element_type=None):
        super().__init__(
            model='cube', color=color.dark_gray, scale=(1, 2, 1),
        ...
        self.elemental_indicator = Entity(parent=self, model='sphere', scale=0.3, position=(0, 1.2, 0), enabled=False)

        self.health_bar_bg = Entity(parent=self, model='quad', color=color.black, scale=(1.2, 0.12), position=(0, 1.5, 0), billboard=True)
        self.health_bar = Entity(parent=self, model='quad', color=color.red, scale=(1.1, 0.1), position=(0, 1.5, 0), billboard=True)

    def take_damage(self, damage, element, reaction=None):
        self.hp -= damage
        self.health_bar.scale_x = (self.hp / self.max_hp) * 1.1

        dmg_text = Text(f"{int(damage)}", position=self.screen_position, origin=(0,0), scale=2, color=color.white)
        if reaction:
            dmg_text.color = color.orange
            reaction_text = Text(reaction, position=self.screen_position + Vec3(0, -0.05, 0), origin=(0,0), scale=2.5, color=color.orange)
    ...
    def update(self):
        if not player.active_character or player.active_character.is_fallen: return
...
class UIManager:
    def __init__(self):
        ...
        self.hp_bar = Entity(parent=camera.ui, model='quad', color=color.green, scale=(0.4, 0.02), position=(-0.65, 0.45), origin=(-0.5, 0))
        self.stamina_bar = Entity(parent=camera.ui, model='quad', color=color.yellow, scale=(0.4, 0.02), position=(-0.65, 0.42), origin=(-0.5, 0))

        self.char_icons = []
        for i, (name, data) in enumerate(CHARACTERS.items()):
            icon = Button(text=str(i+1), color=data['color'], scale=0.06, position=(-0.85 + i*0.07, -0.4))
        ...
        self.interaction_prompt = Text('Press F to interact', position=(0, -0.2), origin=(0,0), scale=1.5, enabled=False)
...
class Waypoint(Entity):
    def __init__(self, position):
        super().__init__(
            model='diamond', color=color.cyan, scale=(1, 3, 1),
            position=position, shader=lit_with_shadows_shader
        )
        self.glow = Entity(parent=self, model='sphere', color=color.rgba(0,255,255,100), scale=3, add_to_scene_entities=False)
        self.glow.animate_scale(3.5, duration=2, curve=curve.in_out_sine, loop=True)

class Tree(Entity):
    def __init__(self, position):
        super().__init__(
            model='cube', color=color.brown, scale=(1, 8, 1),
            position=position, collider='box', shader=lit_with_shadows_shader
        )
        Entity(
            parent=self, model='sphere', color=color.green,
            scale=(6, 4, 6), position=(0, 2.5, 0), shader=lit_with_shadows_shader
        )
